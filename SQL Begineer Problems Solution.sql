...
-- We have a table called Shippers. Return all the fields from all the shipper

select * from shippers;

-- 2. Certain fields from Categories
-- In the Categories table, selecting all the fields using this SQL:  …will return 4 columns. We only want to see two columns, CategoryName and Description
...
-- 3. Sales Representatives We’d like to see just the FirstName, LastName, and HireDate of all the
...
title
from employees
where title like "Sales Repre%";

-- 4. Sales Representatives in the United States Now we’d like to see the same columns as above, but only for those
...
SELECT first_name, last_name, hire_date  FROM northwind.employees where title like "%Sales Representative%" and country ="USA";
...
select  order_id, employee_id from orders where employee_id = 5;

select 
count(order_id) as orders,  -- count of orders 
employee_id
from orders
where employee_id = 5;
...
-- 7. Products with “queso” in ProductName In the products table, we’d like to see the ProductID and ProductName
...
-- 8. Orders shipping to France or Belgium Looking at the Orders table, there’s a field called ShipCountry. Write a
-- query that shows the OrderID, CustomerID, and ShipCountry for the orders where the ShipCountry is either France or Belgium
...
-- American countries that happen to be in the Orders table: Brazil Mexico Argentina Venezuela
...
-- 10. Employees, in order of age For all the employees in the Employees table, show the FirstName,
-- LastName, Title, and BirthDate. Order the results by BirthDate, so we have the oldest employees first.

select last_name, first_name , title, birth_date from employees order by birth_date;
...
select  last_name, first_name, date(birth_date) as dateonlyofbirth from employees;

-- 12. Employees full name Show the FirstName and LastName columns from the Employees table,
-- and then create a new column called FullName, showing FirstName and LastName joined together in one column, with a space in-between.

select first_name ,
 last_name , concat(first_name ," " , last_name) as FullName   from employees;
...
-- 14. How many customers? How many customers do we have in the Customers table? Show one
...
select min(order_date) from orders; -- to find the first order date
select order_date from orders order by order_date; --  to find orders in ascending order 

--  16. Countries where there are customers Show a list of countries where the Northwind company has customers

select distinct(country) from customers;  -- 21 countries has Northwind customers

-- 17. Contact titles for customers Show a list of all the different values in the Customers table for
...
from customers 
group by contact_title  order by total_contact_title desc;
...
-- 19. Orders and the Shipper that was used We’d like to show a list of the Orders that were made, including the
-- Shipper that was used. Show the OrderID, OrderDate (date only), and CompanyName of the Shipper, and sort by OrderID.
-- In order to not show all the orders (there’s more than 800), show only those rows with an OrderID of less than 10300

select   
orders.order_id, 
date(orders.order_date) as orderdateonly , 
suppliers.company_name 
from suppliers
join orders on orders.ship_via= suppliers.supplier_id
